...
marker = 'RHKEARTH_AIS_GLOBAL_STATION_SEED_V5'

helper_pattern = re.compile(
    r"function currentAisViewportBbox\(\) \{.*?\n\}\n\nfunction liveApiUrl\(\) \{.*?\n\}",
    re.S,
)
helper = r'''// RHKEARTH_AIS_GLOBAL_STATION_SEED_V5
// Open Waters anonymous /v1/vessels requests are area-capped, so a camera-bounded
// query can never create an immediate worldwide picture. Seed globally from the
// public station index + station detail snapshots, then use legal 9° tiles only
// for ongoing live refresh. All rows are retained/deduped by MMSI.
const AIS_TILE_DEG = 9;
const AIS_TILE_BATCH = 12;
const AIS_ROW_MAX_AGE_SEC = 45 * 60;
const AIS_STATIONS_URL = 'https://ais.openwaters.io/v1/stations';
const aisGlobalRows = new Map();
let aisTileCursor = 0;
let aisGlobalSeedComplete = false;
let aisGlobalSeedPromise = null;
...
function aisRowsFromGeo(geo) {
  const features = Array.isArray(geo?.features)
    ? geo.features
    : Array.isArray(geo?.vessels?.features)
      ? geo.vessels.features
      : [];
...
async function fetchAisTile(tile, signal) {
  ...
  return aisRowsFromGeo(await response.json());
}

function aisStationDetailUrl(stationId) {
  const safePath = String(stationId || '')
    .split('/')
    .map((part) => encodeURIComponent(part))
    .join('/');
  return `${AIS_STATIONS_URL}/${safePath}`;
}

async function fetchAisStationRows(stationId, signal) {
  const response = await fetch(aisStationDetailUrl(stationId), {
    signal,
    cache: 'no-store',
    mode: 'cors',
  });
  if (!response.ok) throw new Error(`AIS station HTTP ${response.status}`);
  return aisRowsFromGeo(await response.json());
}

async function fetchGlobalAisSeed(signal) {
  if (aisGlobalSeedComplete) return Array.from(aisGlobalRows.values());
  if (aisGlobalSeedPromise) return aisGlobalSeedPromise;

  aisGlobalSeedPromise = (async () => {
    const response = await fetch(AIS_STATIONS_URL, {
      signal,
      cache: 'no-store',
      mode: 'cors',
    });
    if (!response.ok) throw new Error(`AIS stations HTTP ${response.status}`);
    const catalog = await response.json();
    const stationIds = (Array.isArray(catalog) ? catalog : [])
      .map((station) => String(station?.station || '').trim())
      .filter(Boolean);
    if (!stationIds.length) throw new Error('AIS global station catalog empty');

    // Fan out immediately. Station detail is not bounded by the operator's
    // camera viewport, so successful responses seed vessels worldwide at once.
    const settled = await Promise.allSettled(
      stationIds.map((stationId) => fetchAisStationRows(stationId, signal)),
    );
    const fresh = [];
    let fulfilled = 0;
    for (const result of settled) {
      if (result.status === 'fulfilled') {
        ...
        console.warn('[Data:ais] global station seed failed', result.reason);
      }
    }
    mergeGlobalAisRows(fresh);
    // If most station snapshots succeeded, treat the seed as complete. Any
    // misses are filled by normal tile refreshes instead of re-fanning every poll.
    aisGlobalSeedComplete = fulfilled >= Math.max(1, Math.floor(stationIds.length * 0.75));
    return Array.from(aisGlobalRows.values());
  })();

  try {
    return await aisGlobalSeedPromise;
  } finally {
    aisGlobalSeedPromise = null;
  }
}

async function fetchGlobalAisBatch(signal) {
  if (!aisGlobalSeedComplete) {
    try {
      await fetchGlobalAisSeed(signal);
    } catch (error) {
      console.warn('[Data:ais] worldwide startup seed unavailable', error);
    }
  }
  ...
  for (const result of settled) {
    ...
    else console.warn('[Data:ais] tile refresh failed', result.reason);
  }
...
load_replacement = r'''\1    const signal = typeof AbortSignal.any === 'function'
      ? AbortSignal.any([requestController.signal, AbortSignal.timeout(20000)])
...
old_builds = [
    "  buildTag: 'RHKEARTH_AIS_ANON_AREA_LIMIT_V3',",
    "  buildTag: 'RHKEARTH_AIS_GLOBAL_TILE_CACHE_V4',",
]
new_build = "  buildTag: 'RHKEARTH_AIS_GLOBAL_STATION_SEED_V5',"
...
for needle in [marker, 'AIS_STATIONS_URL', 'fetchGlobalAisSeed', 'fetchAisStationRows', 'fetchGlobalAisBatch', 'mergeGlobalAisRows', new_build]:
    if needle not in patched:
        raise SystemExit('AIS worldwide startup contract missing: ' + needle)
print('RHKEARTH AIS repaired: worldwide station seed first, tiled live refresh second')
