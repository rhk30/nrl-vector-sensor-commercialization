...
[
  ...
  ['hero-media.css','hero-media.css?v=2'],
...
// The opening sequence is deliberately passive: one scenario at a time, no
// click-to-select UI. Copy distinguishes patent-described embodiments from
// broader evaluation contexts.
const scenes=[
...
const loadHeroMedia=()=>import('./hero-media.js?v=2').catch(err=>console.warn('RHKEARTH cover media fallback:',err));
...
const loadPatentStrict=()=>import('./patent-strict.js?v=1').catch(err=>console.warn('RHKEARTH patent-strict layer fallback:',err));
const load3D=()=>import('./mission3d.js?v=2').catch(err=>console.warn('RHKEARTH 3D demonstrator fallback:',err));
function loadEnhancements(){
  loadHeroMedia();loadHeroPolish();
  loadSensor().then(loadPatentAccuracy).then(loadPatentStrict);
  load3D();
}
